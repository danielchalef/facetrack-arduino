...
servoTiltPosition = 60  # initial tilt position

TILT_PIN = 9
PAN_PIN = 10

board = a.Arduino('9600', port="/dev/ttyACM1")
...
def detect_and_draw(img, cascade):
    ...
    if(cascade):
        ...
        t = cv.GetTickCount() - t
        #print("Completed Detect in {0}".format(t))
        if faces:
...
def move(SERVO_PIN, angle):
    if (min_pwm <= angle <= max_pwm):
        current_angle = servo.read(SERVO_PIN)
        print("Activiting {0}. Was {1}. Sending {2}".format(
            SERVO_PIN, current_angle, angle))
...
def np_array_to_iplimage(source):
    ...
    bitmap = cv.CreateImageHeader(
        (source.shape[1], source.shape[0]), cv.IPL_DEPTH_8U, 3)
...
if __name__ == '__main__':
    ...
    if capture:
        ...
        while True:
            ...
            frame = np_array_to_iplimage(
                rotate_about_center(np.asarray(frame[:, :]), -90))
            ...
            if midFace is not None:
                ...
                # Find out if the X component of the face is to the left of the
                # middle of the screen.
                if(midFaceX < (midScreenX - midScreenWindow)):
                    # Update the pan position variable to move the servo to the
                    # right.
                    servoPanPosition += panStepSize
                    print str(midFaceX) + " > " + str(midScreenX) + " : Pan Right : " + str(servoPanPosition)
                # Find out if the X component of the face is to the right of
                ...
                # Find out if the Y component of the face is below the middle
                # of the screen.
                if(midFaceY < (midScreenY - midScreenWindow)):
                    if(servoTiltPosition <= max_pwm):
                        # Update the tilt position variable to lower the tilt
                        # servo.
                        servoTiltPosition += tiltStepSize
                        print str(midFaceY) + " > " + str(midScreenY) + " : Tilt Down : " + str(servoTiltPosition)
                # Find out if the Y component of the face is above the middle
                # of the screen.
                elif(midFaceY > (midScreenY + midScreenWindow)):
                    if(servoTiltPosition >= 1):
                        # Update the tilt position variable to raise the tilt
                        # servo.
                        servoTiltPosition -= tiltStepSize
